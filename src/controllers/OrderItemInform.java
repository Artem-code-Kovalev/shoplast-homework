...
@WebServlet("/OrderItemInform")
public class OrderItemInform extends HttpServlet {
	private static final long serialVersionUID = 1L;

    public OrderItemInform() {
    }
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (BaseConnection.UserExist(request, response)){
	        ...
	        try {
				request.setAttribute("OrderGoods", BaseConnection.getOrderItems(IdOrder)); // получаем список товаров в заказе по idOrder
