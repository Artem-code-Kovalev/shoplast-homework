...
<html>
	...
	<script src="http://code.jquery.com/jquery-1.8.3.js"> </script><!-- подкл. jquery -->
		<script>
		function Ready(id){
				var dataStr ="idOrder="+id;//строка запроса	
				$.ajax(
						{
							type:"Post",
							...
							success:function(answer){
								window.location='ChangeOrderStatus';
							}
						}		
					);
				};
		...
		<tr><th colspan="8" class="con">Информация о заказах:</th></tr>
			<tr>
				...
				<tr>
					...
					<td>${i.phone}</td>
					<td>${i.status}</td>
					<td><button class="enter" onclick="Ready(${i.id})">Готово</button></td>	
				</tr>	
			</c:forEach>
		</table>
	</body>
